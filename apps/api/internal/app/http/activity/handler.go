...
type useCase interface {
	...
	Update(ctx context.Context, actorID uuid.UUID, role model.Role, activityID uuid.UUID, req dto.UpdateActivityRequest) (*dto.ActivityDetailView, error)
...
func (h *Handler) Update(w http.ResponseWriter, r *http.Request) {
	...
	role := model.Role(middleware.Role(r.Context()))
	res, err := h.uc.Update(r.Context(), middleware.UserID(r.Context()), role, id, req)
	if err != nil {
		response.Error(w, err)
		return
	}
